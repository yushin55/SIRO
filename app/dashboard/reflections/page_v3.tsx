...
export default function ReflectionsPage() {
  ...
  const { data: recentLogs } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('/api/reflections/micro?limit=7', {
        headers: {
  ...
  const { data: weekStats } = useQuery({
    queryKey: ['week-stats'],
    queryFn: async () => {
      const response = await fetch('/api/reflections/stats?period=week', {
        headers: {
          'x-user-id': localStorage.getItem('x-user-id') || '',
        },
      });
  ...
  const needsBaseline = !userData?.data?.baseline_mood;
